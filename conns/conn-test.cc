...
inline void add_s_certs(ssl::context &ctx) {
  ...
  ctx.set_password_callback(
      [](std::size_t, ssl::context_base::password_purpose) { return "test"; });
...
class Rcvr {
public:
  virtual ~Rcvr() = default;
  virtual void Body(const std::string &) = 0;
  virtual void Header(const std::string &, const std::string &) = 0;
  virtual void Method(const std::string &) = 0;
};

void srv_session(Rcvr &rcvr) {
  try {
    auto const address = asio::ip::make_address("0.0.0.0");
    const std::size_t port = 443;
    ...
    rcvr.Method(req.method_string());
    for (const auto &field : req.base()) {
      rcvr.Header(field.name_string(), field.value());
    }
...
class MockRcvr : public Rcvr {
public:
  MOCK_METHOD(void, Body, (const std::string &));
  MOCK_METHOD(void, Header, (const std::string &, const std::string &));
  MOCK_METHOD(void, Method, (const std::string &));
};

template <class ConnMethod, class... ConnHead>
static inline void Apply(ConnMethod &&method, ConnHead... head) {
  auto conn = Connection::Https("localhost");
  auto req = (conn.get()->*std::forward<ConnMethod>(method))("/dummy");
  (void)std::initializer_list<std::nullptr_t>{
      ((req.get()->*head.first)(head.second), nullptr)...};
  auto res = req->Do();
  EXPECT_EQ(res.body, "dummy");
}

template <class ConnMethod> static inline void Apply(ConnMethod &&method) {
  auto conn = Connection::Https("localhost");
  auto req =
      (conn.get()->*std::forward<ConnMethod>(method))("/dummy", "payload");
  auto res = req->Do();
  EXPECT_EQ(res.body, "dummy");
}

TEST(ConnTest, GetAndHeaders) {
  MockRcvr rcvr;
  EXPECT_CALL(rcvr, Header("Host", "localhost")).Times(1);
  EXPECT_CALL(rcvr, Header("User-Agent", "agent")).Times(1);
  EXPECT_CALL(rcvr, Header("Authorization", "authorization")).Times(1);
  EXPECT_CALL(rcvr, Header("Content-Type", "content_type")).Times(1);
  EXPECT_CALL(rcvr, Body("")).Times(1);
  EXPECT_CALL(rcvr, Method("GET")).Times(1);
  std::thread srv_th{std::bind(srv_session, std::ref(rcvr))};
  Apply(&Connection::Get, std::make_pair(&Request::Agent, "agent"),
        std::make_pair(&Request::Authorization, "authorization"),
        std::make_pair(&Request::ContentType, "content_type"));
  srv_th.join();
}
...
TEST(ConnTest, Post) {
  MockRcvr rcvr;
  EXPECT_CALL(rcvr, Header("Host", "localhost")).Times(1);
  EXPECT_CALL(rcvr, Header("Content-Length", "7")).Times(1);
  EXPECT_CALL(rcvr, Body("payload")).Times(1);
  EXPECT_CALL(rcvr, Method("POST")).Times(1);
  std::thread srv_th{std::bind(srv_session, std::ref(rcvr))};
  Apply(&Connection::Post);
  srv_th.join();
}
...
TEST(ConnTest, Patch) {
  ...
  EXPECT_CALL(rcvr, Header("Content-Length", "7")).Times(1);
  EXPECT_CALL(rcvr, Body("payload")).Times(1);
  EXPECT_CALL(rcvr, Method("PATCH")).Times(1);
  std::thread srv_th{std::bind(srv_session, std::ref(rcvr))};
